...
class User(db.Model, UserMixin):
  ...
  profile_verified = db.Column(db.Boolean, nullable = True)
  user_details = db.relationship("UserDetails", backref = "user", uselist = False)
  professional_qualifications = db.relationship("ProfessionalQualification", backref = "professional", uselist = False)
...
class ProfessionalQualification(db.Model):
  __tablename__  = "professional_qualification"
  user_id = db.Column(db.Integer, db.ForeignKey("user.user_id", ondelete = "CASCADE"), primary_key = True)
...
class ServiceRequest(db.Model):
  ...
  customer_id = db.Column(db.Integer, db.ForeignKey("user.user_id", ondelete = "CASCADE"))
  ...
  service_status = db.Column(db.String(10), default = "Pending", nullable = False)
...
class ProfessionalAction(db.Model):
  ...
  professional_id = db.Column(db.Integer, db.ForeignKey("user.user_id"))
  action_type = db.Column(db.String(10), nullable = False)
  
  __table_args__ = (
    db.PrimaryKeyConstraint(
      "service_request_id", "professional_id",
      name = "primary_key_constraint"
      ),
    db.CheckConstraint(
      "action_type IN ('Pending', 'Accepted', 'Rejected', 'Cancelled', 'Closed')", 
